...
class ProductDetails : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Glide.with(applicationContext)
            .load("http://192.168.1.5/doan3/uploads/product/${intent.getStringExtra("image").toString()}") // URL của ảnh
            .thumbnail(0.5f)
...
//        val imageView = findViewById<ImageView>(R.id.img_product_detail)
        Toast.makeText(this, "http://192.168.1.5/doan3/uploads/product/${intent.getStringExtra("image")}", Toast.LENGTH_SHORT).show()
        ...
        findViewById<Button>(R.id.btn_add_to_cart).setOnClickListener(){
            ...
            var url = "http://192.168.1.5/doan3/addcart.php?id_user=${UserStore(applicationContext).getId()}&id_product=${intent.getStringExtra("proid")}&amount=${findViewById<TextView>(R.id.txt_product_amount).text}"
